...
func RegisterJobTools(s *server.MCPServer, sippy client.Sippy) {
	s.AddTool(
		mcp.NewTool("get_job_report",
			...
			mcp.WithString("job_name", mcp.Description("Job name substring filter")),
	...
	s.AddTool(
		mcp.NewTool("get_job_runs",
			mcp.WithDescription("Use to get results, timings, and risk analysis of recent job runs"),
			mcp.WithReadOnlyHintAnnotation(true),
			mcp.WithDestructiveHintAnnotation(false),
			mcp.WithIdempotentHintAnnotation(true),
			mcp.WithOpenWorldHintAnnotation(true),
	...
	s.AddTool(
		mcp.NewTool("get_job_run_summary",
			mcp.WithDescription("Use to get test failures and cluster operator status of a single job run"),
			mcp.WithReadOnlyHintAnnotation(true),
			mcp.WithDestructiveHintAnnotation(false),
			mcp.WithIdempotentHintAnnotation(true),
			mcp.WithOpenWorldHintAnnotation(true),
